...
Sub Totales_x_cuoc__x_Persona_x_vto_con_total_general()
    Dim nFilas As Long
    ...
    Sheets("HISTORICO").Select
      ...
      If ultDoc = Cells(i, 5).Value Then
       ...
       If i = 2 Then
         ...
         Else
           ...
           For j = pos To nFilasCont
            If Sheets("Total x cuoc x Persona").Cells(j, 4).Value <> cuoc Or Sheets("Total x cuoc x Persona").Cells(j, 7).Value <> vto Then
       ...
       Else
              ...
              wsTotal.Cells(filaTotal, 5).Value = totalimportemas
              wsTotal.Cells(filaTotal, 6).Value = totalimportemenos
              Sheets("Total x cuoc x Persona").Select
              wsTotal.Cells(filaTotal + 1, 1).Value = Sheets("HISTORICO").Cells(i, 2).Value
              wsTotal.Cells(filaTotal + 1, 2).Value = Sheets("HISTORICO").Cells(i, 5).Value

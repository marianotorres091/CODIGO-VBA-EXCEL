...
Sub Registros_duplicados()
    ...
    For i = 2 To limite
     ...
     If Cells(i, nColumnas + 1).Value <> "Repetido" Then
        ...
        For j = (i + 1) To nFilas
            If valorDoc = Cells(j, 5).Value And valorCouc = Cells(j, 8).Value And valorRj = Cells(j, 10).Value And valorunidad = Cells(j, 11).Value And valorimporte = Cells(j, 12).Value And valorVto = Cells(j, 15).Value Then
